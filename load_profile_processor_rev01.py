...
def data_processing(LOGGER_FILE,PVSYST_FILE,OUTPUT_DIR,COMPANY,CAPACITY,DESC):
	...
	DESC        = '_'+DESC if len(DESC)!=0 else DESC
	OUTPUT_FILE = OUTPUT_DIR+f'/energy_graph_load_profile_{COMPANY}_{CAPACITY:.3f}kWp{DESC}.xlsx'
	...
	plt.title('Load Profile for \n'+COMPANY+'_'+str(CAPACITY).zfill(3)+'kwp \nfrom '
			  +str(final.date.values[0])+' to '+str(final.date.values[-1]), fontsize=16, linespacing=2)
